...
import { Menu, X, Facebook, Phone, LogOut } from 'lucide-react';
...
export function AdminLayout({ children, currentPage, onNavigate, onLogout }: AdminLayoutProps) {
  ...
  const navItems = [
    ...
    { id: 'series', label: 'Quản lý Dòng xe' },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`transition-all duration-300 ${
                    currentPage === item.id || (item.id === 'products' && currentPage === 'productDetail')
                      ? 'text-green-600'
      ...
      {/* Main Content */}
      <div className={currentPage === 'productDetail' ? '' : 'p-4 md:p-8 max-w-7xl mx-auto'}>
        {children}
      </div>
    </div>
  );
