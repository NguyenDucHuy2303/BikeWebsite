...
import { Config, PageInfo, pageInfoList, initialConfigs, Banner } from '../../mockData/adminData';
...
export function AdminDashboard({ totalProducts, totalSeries, totalNews, banners, onNavigate, onSaveBanners }: AdminDashboardProps) {
  ...
  const [editConfig, setEditConfig] = useState<{ type: 'contact' | 'banner', data: Config | null }>({ type: 'contact', data: null });
  ...
  const quickLinks = [
    { label: 'Quản lý Sản phẩm', icon: Package, page: 'products', color: 'bg-green-500' },
    { label: 'Quản lý Dòng xe', icon: Layers, page: 'series', color: 'bg-green-600' },
    { label: 'Quản lý Tin tức', icon: Newspaper, page: 'news', color: 'bg-green-700' },
  ];
  ...
  const updateContactConfig = (configId: number, type: string, value: string) => {
  ...
  const handleBannerImageChange = (index: number, image: string) => {
    setEditingBanners(prev => prev.map((banner, i) =>
      i === index ? { ...banner, image } : banner
    ));
  };

  const handleBannerLinkChange = (index: number, link: string) => {
    setEditingBanners(prev => prev.map((banner, i) =>
      i === index ? { ...banner, link } : banner
    ));
  };

  const handleAddBanner = () => {
    const newId = Math.max(...editingBanners.map(b => b.id), 0) + 1;
    setEditingBanners(prev => [...prev, {
      id: newId,
      image: '',
      link: '',
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Quản lý các thiết lập chung của website</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={openBannerEdit} variant="outline" className="gap-2">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button onClick={openContactEdit} variant="outline" className="gap-2">
                <Pencil className="w-4 h-4" />
                Chỉnh sửa
              </Button>
            </div>
      ...
      <Dialog open={showBannerDialog} onOpenChange={cancelBannerEdit}>
        <DialogContent className="w-[95vw] sm:!w-[90vw] sm:!max-w-[90vw] md:!w-[85vw] md:!max-w-[1200px] max-h-[90vh] p-0">
          <DialogHeader className="px-6 pt-6">
            ...
            <DialogDescription>
              Thêm hoặc chỉnh sửa banner carousel. Mỗi banner phải có cả hình ảnh và link.
            </DialogDescription>
          </DialogHeader>
          ...
          <ScrollArea className="max-h-[calc(90vh-120px)] px-6">
            <div className="space-y-6 pb-6">
                  ...
                  <Card key={banner.id} className="relative">
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <ImageUpload
                          value={banner.image}
                          onChange={(url) => handleBannerImageChange(index, url)}
                        />
                      </div>
              ...
              <div className="flex justify-end gap-2 pt-4 border-t">
                <Button type="button" variant="outline" onClick={cancelBannerEdit}>
                  Hủy
                </Button>
      ...
      <Dialog open={showContactDialog} onOpenChange={cancelContactEdit}>
        <DialogContent className="w-[95vw] sm:!w-[90vw] sm:!max-w-[90vw] md:!w-[85vw] md:!max-w-[800px] max-h-[90vh] p-0">
          <DialogHeader className="px-6 pt-6">
            ...
            <DialogDescription>
              Thêm, sửa hoặc xóa thông tin liên hệ. Tất cả thông tin phải có giá trị.
            </DialogDescription>
          </DialogHeader>
          ...
          <ScrollArea className="max-h-[calc(90vh-120px)] px-6">
            <div className="space-y-6 pb-6">
              ...
              {/* Submit Buttons */}
              <div className="flex justify-end gap-2 pt-4 border-t">
                <Button type="button" variant="outline" onClick={cancelContactEdit}>
                  Hủy
                </Button>
